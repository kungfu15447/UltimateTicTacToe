...
public class EggplantAI implements IBot
{
    ...
    public IMove doMove(IGameState state)
    {
        ...
        List<IMove> someMoves = state.getField().getAvailableMoves();
        ...
        if (moveToBeMade == null)
        {
            while (!isGoodMove)
            {
                ...
                if (moveToBeMade == null && someMoves.size() > 0)
                {
                    moveToBeMade = someMoves.get(RAND.nextInt(someMoves.size()));
                    
                }
                
                if (checkIfEnemyWinsNextBoard(state, moveToBeMade)) {
                    someMoves.remove(moveToBeMade);
                    moveToBeMade = null;
                } else {
                    isGoodMove = true;
                }
    ...
    private boolean checkIfEnemyWinsNextBoard(IGameState state, IMove move)
    {
        ...
        for (IMove move2 : availableMoves)
        {
            ...
            int localX = move2.getX() % 3;
            int localY = move2.getY() % 3;
